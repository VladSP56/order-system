...
def init_db():
    conn = sqlite3.connect('business_order.db') # Используем одно имя базы данных
    cur = conn.cursor()
    cur.execute("""
...
def add_order():
    conn = sqlite3.connect('business_order.db') #Теперь та же база данных
    cur = conn.cursor()
...
(customer_name_entry.get(), order_details_entry.get()))
    ...
    view_orders() # Обновляем список заказов после добавления
...
complete_button = tk.Button(app, text="Завершен заказ", command=complete_order)
complete_button.pack() # Кнопка завершения заказа
...
init_db()
view_orders() #Показываем заказы при запуске
